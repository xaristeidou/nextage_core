...
class DynamicPlanningScene:
    ...
    def __init__(self):
        ...
        self.planning_scene_service = rospy.ServiceProxy(
    ...
    def publish_planning_scene(self):
        ...
        if self.check_transforms('small_hob_frame'):
            ...
            (small_hob.pose.orientation.x,
             small_hob.pose.orientation.y,
             small_hob.pose.orientation.z,
             small_hob.pose.orientation.w) = (0.0, 0.0, 0.0, 1.0)
            (small_hob.pose.position.x,
             small_hob.pose.position.y,
             small_hob.pose.position.z) = (0.0,0.0,0.0) 
            self.planning_scene_interface.add_mesh(
                "small_hob",
                small_hob,
                os.path.join(local_mesh_repo_dir,'meshes/small_hob_pivot_in_center.stl')
            )
        ...
        if self.check_transforms('medium_hob_frame'):
            ...
            (medium1_hob.pose.orientation.x,
             medium1_hob.pose.orientation.y,
             medium1_hob.pose.orientation.z,
             medium1_hob.pose.orientation.w) = (0.0, 0.0, 0.0, 1.0)
            (medium1_hob.pose.position.x,
             medium1_hob.pose.position.y,
             medium1_hob.pose.position.z) = (0.0, 0.0, 0.0)
            self.planning_scene_interface.add_mesh(
                "medium_hob",
                medium1_hob,
                os.path.join(local_mesh_repo_dir,'meshes/small_hob_pivot_in_center.stl')
        ...
        if self.check_transforms('big_hob_frame'):
            ...
            (big_hob.pose.orientation.x,
             big_hob.pose.orientation.y,
             big_hob.pose.orientation.z,
             big_hob.pose.orientation.w) = (0.0, 0.0, 0.0, 1.0)
            (big_hob.pose.position.x,
             big_hob.pose.position.y,
             big_hob.pose.position.z) = (0.0, 0.0, 0.0)
            self.planning_scene_interface.add_mesh(
                "big_hob",
                big_hob,
                os.path.join(local_mesh_repo_dir,'meshes/small_hob_pivot_in_center.stl')
        ...
        if self.check_transforms('pcb_frame'):
            ...
            self.planning_scene_interface.add_mesh(
                "pcb",
                big_hob, 
                os.path.join(local_mesh_repo_dir,'meshes/pcb.stl')
            )
